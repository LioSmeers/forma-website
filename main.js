...
const navButtons = document.querySelectorAll("header [data-target], .hero-section [data-target]");
...
function scrollToSection(id) {
  ...
  const runScroll = () => section.scrollIntoView({ behavior: "smooth", block: "start" });
...
function updateScrollState() {
  ...
  const activeId = ["diensten", "pakketten", "contact"].reduce((current, id) => {
    const section = document.getElementById(id);
    return section && section.getBoundingClientRect().top <= 180 ? id : current;
  }, "");
...
function openSpotlight(packageKey) {
  ...
  spotlight.querySelector(".spotlight-description").textContent = item.description;
  ...
  benefitList.innerHTML = item.benefits
    .map((benefit) => `<li><span class="checkmark">✓</span><span>${benefit}</span></li>`)
    .join("");
...
spotlight.querySelector("[data-target='contact']").addEventListener("click", () => {
  closeSpotlight();
